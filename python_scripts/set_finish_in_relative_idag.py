logger.info("Starting Python script: set_finish_in_relative_idag")

# Fetch the finish time sensor
finish_time_state = hass.states.get("sensor.elpris_lavest_4t_vaskemaskine_idag")

if finish_time_state is None:
    logger.error("Sensor 'sensor.elpris_lavest_4t_vaskemaskine_idag' is NOT available.")
    hass.states.set("sensor.finish_in_relative", -1, {
        "unit_of_measurement": "seconds",
        "friendly_name": "Finish In Relative",
        "error": "Missing sensor.elpris_lavest_4t_vaskemaskine_idag"
    })
else:
    finish_time = finish_time_state.state
    if not finish_time:
        logger.error("Sensor 'sensor.elpris_lavest_4t_vaskemaskine_idag' is empty or null.")
        hass.states.set("sensor.finish_in_relative", -1, {
            "unit_of_measurement": "seconds",
            "friendly_name": "Finish In Relative",
            "error": "Empty state"
